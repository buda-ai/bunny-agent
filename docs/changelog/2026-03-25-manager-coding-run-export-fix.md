...
- Restored missing `@bunny-agent/manager` exports in `packages/manager/src/coding-run.ts`:
  `buildDefaultCodingRunExec`, `buildDefaultDaemonCodingRunExecCommand`,
  `BUNNY_AGENT_DAEMON_HEALTHZ_PATH`, and `probeSandagentDaemonHealth`.
- Added `DaemonCodingRunExecParams` and re-exported it from
  `packages/manager/src/index.ts` for `@bunny-agent/sandbox-sandock` compatibility.
...
- Fixed provider stream parsing in `packages/sdk/src/provider/bunny-agent-language-model.ts`:
...
- `@bunny-agent/daemon` no longer depends on `@bunny-agent/manager`; coding-run env merge
