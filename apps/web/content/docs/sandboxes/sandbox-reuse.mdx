...
Pass a `sandboxId` to reuse an existing sandbox. `attach()` tries to reconnect first; on failure it creates a new one.
...
- If the sandbox expired or was deleted, `attach()` falls back to creating a new one
