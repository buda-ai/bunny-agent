...
import { ok, resolveVolumeRoot, resolveUnderRoot, ensureDir, AppError } from "../utils.js";

const exec = promisify(execFile);

const ALLOWED_GIT_COMMANDS = new Set([
  "status", "log", "diff", "show", "branch", "checkout",
  "add", "commit", "reset", "init", "rev-parse",
  "fetch", "pull", "push", "merge", "rebase", "remote", "tag", "ls-files",
]);
...
async function runGit(cwd: string, args: string[]): Promise<GitCommandResponse> {
  try {
    const { stdout, stderr } = await exec("git", ["-C", cwd, ...args], { maxBuffer: 10 * 1024 * 1024 });
    return { stdout, stderr, code: 0 };
  } catch (err: any) {
    if (err.code === "ENOENT") throw new AppError(500, "git not found");
    return { stdout: err.stdout ?? "", stderr: err.stderr ?? "", code: err.code ?? 1 };
  }
}

export async function gitStatus(state: AppState, body: { volume?: string; repo: string }) {
...
export async function gitExec(state: AppState, body: { volume?: string; repo: string; args: string[] }) {
...
export async function gitClone(state: AppState, body: {
  volume?: string; repo_parent: string; url: string;
  branch?: string; depth?: number; target_dir?: string; list_files_limit?: number;
}) {
  const root = resolveVolumeRoot(state, body.volume);
  ...
  // Infer repo path
  const dirName = body.target_dir ?? body.url.replace(/\/$/, "").split(/[/:]/).pop()!.replace(/\.git$/, "");
  const repoPath = resolveUnderRoot(parent, dirName);

  // List tracked files
  const lsResult = await runGit(repoPath, ["ls-files"]).catch(() => ({ stdout: "", stderr: "", code: 1 }));
...
export async function gitInit(state: AppState, body: { volume?: string; repo: string; initial_branch?: string }) {
  const root = resolveVolumeRoot(state, body.volume);
  const repo = resolveUnderRoot(root, body.repo);
