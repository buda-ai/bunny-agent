...
export type FileWithStatus = FileUIPart & {
  id: string;
  uploading?: boolean;
  error?: string;
};
...
export type PromptInputProviderProps = PropsWithChildren<{
  initialInput?: string;
  onFileUpload?: (files: FileUIPart[]) => Promise<FileUIPart[]>;
}>;
...
export function PromptInputProvider({
  ...
  const [attachements, setAttachements] = useState<FileWithStatus[]>([]);
  ...
  const uploadFile = useCallback(
    async (id: string, file: File) => {
      ...
      try {
        ...
        const [uploaded] = await onFileUpload([filePart]);

        // Update with uploaded URL
        setAttachements((prev) =>
          prev.map((f) =>
            f.id === id
              ? { ...f, url: uploaded?.url ?? f.url, uploading: false, error: undefined }
              : f,
          ),
        );
  ...
  const add = useCallback(
    (files: File[] | FileList) => {
      ...
      const newFiles: FileWithStatus[] = incoming.map((file) => ({
        id: nanoid(),
        type: "file" as const,
        url: URL.createObjectURL(file),
        mediaType: file.type,
      ...
      // Trigger immediate upload if handler provided
      if (onFileUpload) {
        for (let i = 0; i < newFiles.length; i++) {
          uploadFile(newFiles[i].id, incoming[i]);
        }
      }
    },
  ...
  const attachments = useMemo<AttachmentsContext>(
    ...
    [attachements, add, remove, clear, retry, openFileDialog],
  );
      ...
      <HoverCardTrigger asChild>
        ...
        >
          <div className="relative size-5 shrink-0">
            {isUploading ? (
              <div className="absolute inset-0 flex size-5 items-center justify-center">
              ...
              <>
                ...
                <Button
                  aria-label={hasError ? "Retry upload" : "Remove attachment"}
                ...
                >
                  <XIcon />
                  <span className="sr-only">{hasError ? "Retry" : "Remove"}</span>
                </Button>
              </>
            )}
          </div>

          <span className={cn("flex-1 truncate", hasError && "text-destructive")}>
            {hasError ? "Upload failed" : attachmentLabel}
          </span>
        </div>
      </HoverCardTrigger>
...
export type PromptInputAttachmentsProps = Omit<HTMLAttributes<HTMLDivElement>, "children"> & {
  children: (attachment: FileWithStatus) => ReactNode;
};
...
export type PromptInputProps = Omit<HTMLAttributes<HTMLFormElement>, "onSubmit" | "onError"> & {
  ...
  /**
   * Optional callback to handle file upload before submission.
   * When provided, this function is called instead of the default base64 conversion.
   * Use this to upload files to S3 or other storage and return URLs.
   * @param files - Array of FileUIPart with blob URLs
   * @returns Promise resolving to array of FileUIPart with uploaded URLs
   */
  onFileUpload?: (files: FileUIPart[]) => Promise<FileUIPart[]>;
  onSubmit: (
...
export const PromptInput = ({
  ...
  const [items, setItems] = useState<FileWithStatus[]>([]);
  const files = usingProvider ? controller.attachments.files : items;

  // Store pending files for retry (local mode)
  const pendingFilesRef = useRef<Map<string, File>>(new Map());
  ...
  const uploadFileLocal = useCallback(
    async (id: string, file: File) => {
      if (!onFileUpload) return;
  ...
  const addLocal = useCallback(
    (fileList: File[] | FileList) => {
      ...
      setItems((prev) => prev.concat(newFiles));

      // Trigger immediate upload if handler provided
      if (onFileUpload) {
        for (let i = 0; i < newFiles.length; i++) {
          uploadFileLocal(newFiles[i].id, capped[i]);
        }
      }
    },
    [matchesAccept, maxFiles, maxFileSize, onError, onFileUpload, uploadFileLocal, items.length],
  );
  ...
  const retry = usingProvider ? (id: string) => controller.attachments.retry(id) : retryLocal;
  ...
  // Cleanup blob URLs on unmount only, not on every files change
  // This prevents blob URLs from being revoked while async operations (like file upload) are in progress
