...
export default function (pi: ExtensionAPI): void {
  ...
  pi.on("agent_end", async (event, ctx) => {
    const mode: GoalAccountingMode =
      completedThisTurnGoalId === null ? "active" : "activeOrComplete";
    ...
    if (
      goal?.status === "active" &&
      shouldQueueGoalContinuationAfterAgentEnd(goal, ctx.hasPendingMessages())
    ) {
      queueHiddenGoalPrompt(
        pi,
        GOAL_CONTINUATION_MESSAGE_TYPE,
  ...
  async function confirmReplaceGoal(
    ...
    const choice = await ctx.ui.select(
      ...
      [REPLACE_GOAL_CHOICE, CANCEL_REPLACE_GOAL_CHOICE],
    );
  ...
  async function maybePromptResumePausedGoal(
    ...
    const choice = await ctx.ui.select(
      `Resume paused goal?\nGoal: ${goal.objective}`,
      [RESUME_GOAL_CHOICE, LEAVE_GOAL_PAUSED_CHOICE],
    );
...
function agentDir(): string {
  return (
    process.env["BUNNY_AGENT_DIR"] ??
    process.env["PI_CODING_AGENT_DIR"] ??
    join(homedir(), ".bunny", "agent")
  );
}
