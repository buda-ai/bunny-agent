...
export async function updateGoal(
  ...
  const requestedStatus =
    update.status ?? (hasObjectiveUpdate ? "active" : undefined);
  ...
  const nextTokenBudget =
    tokenBudget === null ? undefined : (tokenBudget ?? current.tokenBudget);
  const status =
...
function statusAfterExplicitStatusUpdate(
  ...
  if (currentStatus === "budgetLimited" && requestedStatus === "paused")
