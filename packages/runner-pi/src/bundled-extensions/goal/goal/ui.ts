  ...
  switch (goal.status) {
    case "active":
      return {
        color,
        text:
          usageText === null ? "Pursuing goal" : `Pursuing goal (${usageText})`,
      };
    case "paused":
    ...
    case "budgetLimited":
      return {
        color,
        text:
          usageText === null ? "Goal abandoned" : `Goal unmet (${usageText})`,
      };
    case "complete":
      return {
        color,
        text:
          usageText === null ? "Goal achieved" : `Goal achieved (${usageText})`,
      };
  }
}
...
class GoalFooterComponent implements Component {
  ...
  private workingDirectoryLine(width: number): string {
    ...
    if (
      ...
      workingDirectory.startsWith(homeDirectory)
    ) {
